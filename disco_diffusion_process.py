# This is an adaptation of Disco Diffusion; I'm allowing it to be run locally
...
# Add some folders to your Path, so that you can import things later on
import sys
...
#              METHODS
# ====================================
# Below, a number of different methods are defined for use in the rest of the script
...
# This method will launch a run of diffusion
def do_run(args):
    seed = args.seed
    for frame_num in range(args.start_frame, args.max_frames):

        # This used to be a "stop_on_next_loop" parameter that could be tweaked; it apparently made sure that GPU memory wasn't corrupted from cancelling
        # the run halfway through. What does this mean?
        if False:
            break
        ...
        if args.animation_mode == "2D":
            ...
            if frame_num > 0:
                ...
                if resume_run and frame_num == args.start_frame:
                    img_0 = cv2.imread(
                        batchFolder + f"/{args.batch_name}({args.batchNum})_{args.start_frame - 1:04}.png")
                else:
...
def save_settings(args, cur_batch_num):
    ...
    with open(f"{batchFolder}/{args.batch_name}({cur_batch_num})_settings.txt", "w+",
...
class MakeCutoutsDango(nn.Module):
    ...
    def forward(self, input):
        ...
        # The empty dict at the end used to be a dict called "padargs" - I'd imagine this allowed you to add additional arguments to this
        ...
        if self.Overview > 0:
            ...
            # This used to have a reference to "cutout_debug", but I removed it
            if False:
...
# This method will run diffusion
def run_diffusion(arg_dicts):
    ...
    # This sets up the root path
    root_path = 'content'

    # This creates some additional folders - one for initialization images, and another for output images
